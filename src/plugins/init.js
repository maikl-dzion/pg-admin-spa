...
import DataFormMixin from './data_form'
    ...
    Vue.mixin({
      ...
      methods: {
          ...
          switch (actionName) {
            case 'databases' :
              ...
              if (name === 'template1' ||
                  name === 'template0' ||
                  name === 'maikldb' ||
                  name === 'postgres') { return false }
            case 'users' :
              name = item.usename
              if (name === 'postgres' ||
                  name === 'w1user') { return false }
            case 'get_roles' :
              name = item.rolname
              if (name === 'postgres' ||
                name === 'w1user') { return false }
        ...
        queryExecute () {
          ...
          if (!this.sqlCommandType) { this.sqlCommandType = 'query' }
        ...
        setAutoIncName () {
          ...
          for (var fName in fields) {
            if (fields[fName].auto_increment) { this.autoIncName = fName }
          }
        ...
        createTable (callback = null) {
          ...
          this.http(url).then(resp => {
            this.tableName = tableName
            if (callback) { callback(tableName) }
            this.getTableList()
            this.getTableFields(this.tableName)
            this.getTableListSheme()
